...
static void pad_disconnect_check(void)
{
    ...
    if (pad_xram != 0xFFFF)
    {
        if (absolute_time_diff_us(get_absolute_time(), pad_p1_timer) < 0)
            psram[pad_xram + 4] = 0x0F;
        if (absolute_time_diff_us(get_absolute_time(), pad_p2_timer) < 0)
            psram[pad_xram + sizeof(sony_ds4_report_t) + 4] = 0x0F;
    }
}
...
void pad_report(uint8_t dev_addr, uint8_t const *report)
{
    ...
    if (player == 1)
    {
        ...
        if (pad_xram != 0xFFFF)
            memcpy(&psram[pad_xram], report + 1, sizeof(sony_ds4_report_t));
    }
    ...
    if (player == 2)
    {
        ...
        if (pad_xram != 0xFFFF)
            memcpy(&psram[pad_xram + sizeof(sony_ds4_report_t)], report + 1, sizeof(sony_ds4_report_t));
    }
}
