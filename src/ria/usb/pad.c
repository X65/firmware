...
static void pad_reset_xram(int player_idx)
{
    ...
    memcpy(&psram[pad_xram + player_idx * (sizeof(pad_report_t))],
...
void pad_report(uint8_t idx, uint8_t const *report, uint16_t len)
{
    ...
    if (pad_xram != 0xFFFF)
    {
        ...
        memcpy(&psram[pad_xram + player * (sizeof(pad_report_t))],
               &gamepad_report, sizeof(pad_report_t));
    }
}
...
void pad_home_button(uint8_t idx, bool pressed)
{
    ...
    if (pad_xram != 0xFFFF)
    {
        uint8_t *button1 = &psram[pad_xram + player * (sizeof(pad_report_t)) + 3];
        if (pressed)
            *button1 |= (1 << 4);
        else
            *button1 &= ~(1 << 4);
