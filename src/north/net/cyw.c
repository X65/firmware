...
X(STR_ERR_CYW_FAILED_TO_INIT, "?CYW43 failed to init\n")
#undef X
...
void cyw_init(void)
{
    uint32_t clk_sys_khz = clock_get_hz(clk_sys) / KHZ;
    ...
    if (clk_sys_khz > 264000)
        cyw43_set_pio_clkdiv_int_frac8(5, 0);
    else if (clk_sys_khz > 198000)
        cyw43_set_pio_clkdiv_int_frac8(4, 0);
    else if (clk_sys_khz > 132000)
